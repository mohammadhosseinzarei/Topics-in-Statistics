...
# SE for mean response at x0
SE_mean <- sigma_hat * sqrt(1/n + (x0 - x_bar)^2 / Sxx)

CI_mean <- c(y0_hat - t_crit * SE_mean,
             y0_hat + t_crit * SE_mean)

# SE for prediction at x0
SE_pred <- sigma_hat * sqrt(1 + 1/n + (x0 - x_bar)^2 / Sxx)
PI<-c(y0_hat- t_crit *SE_pred,
      y0_hat + t_crit*SE_pred)
